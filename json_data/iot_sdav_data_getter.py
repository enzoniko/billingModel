...
if len(sys.argv) < 5:
    print("Expected use: ./iot_sdav_data_getter.py <commit_id : hash from git> <timestamp : microseconds since epoch> <download camera/lidar : 1/0> <signature> Missing argument 'commit_id' or 'timestamp' or , please add the hash of the commit followed by a space and then the timestamp of the commit as a parameter of this script!")
    sys.exit(1)

commit_id = sys.argv[1]
t0 = int(sys.argv[2])
download_cam_lidar = int(sys.argv[3])
SIGNATURE = int(sys.argv[4])
zip_unit = 4 << 24 | 1
