...
from typing import Tuple, List, Dict, Union, Optional, Any, Protocol
import pandas as pd
import numpy as np
import torch
import pickle
...
class ReducerProtocol(Protocol):
    def fit(self, X: np.ndarray) -> 'ReducerProtocol': ...
...
RESULTS_DIR = "results_experiment5"
MODELS_DIR = "autoencoder_models"
AUTOENCODER_RESULTS_DIR = "autoencoder_results"
RECONSTRUCTION_ERROR_CACHE_DIR = "reconstruction_error_cache"
os.makedirs(RESULTS_DIR, exist_ok=True)
os.makedirs(RECONSTRUCTION_ERROR_CACHE_DIR, exist_ok=True)
...
DIMENSIONALITY_REDUCTION = {
    # Default is now 'none' – no dimensionality reduction. Users can still override via CLI.
    'method': 'none',  # Options: 'none', 'intelligent_pca', 'umap', 'incremental_pca', 'feature_selection', 'hybrid'
...
# Method Selection Configuration - kmeans + baseline + xgboost
METHODS_TO_RUN = ['kmeans', 'baseline', 'hybrid', 'xgboost']  # kmeans + baseline method
...
AUGMENTATION_PARAMS = {
    'min_samples_per_class': 20, # Each class will have at least 20 samples
    'noise_level': 0.02          # Small amount of noise to add
}

# XGBoost Training Configuration (kept here as it's complex to CLI)
XGBOOST_PARAMS = {
    'grid_search_cv': 3,  # 3-fold cross-validation for grid search
    'param_grid': {
...
def generate_reconstruction_error_signals(df: pd.DataFrame, model: VehicleAutoencoder, 
    ...
    # Extract sensor data
    sensor_data = df[SENSORS_FOR_AUTOENCODER].to_numpy()
...
def intelligent_pca_reduction(features: np.ndarray, target_variance: float = 0.95, 
                            max_components: int = 100, min_components: int = 10) -> Tuple[ReducerProtocol, int]:
    """
    ...
    if n_samples < 2:
        raise ValueError("Cannot perform PCA with less than 2 samples.")
    ...
    if max_possible == 0:
        ...
        return DummyPCAReducer(), features.shape[1]

    # Fit PCA with maximum possible components
    pca_full: ReducerProtocol = PCA(n_components=max_possible)
    pca_full.fit(features)
    
    # This attribute is specific to PCA, so we check for it
    if hasattr(pca_full, 'explained_variance_ratio_'):
        # Calculate cumulative explained variance
        cumsum_variance = np.cumsum(pca_full.explained_variance_ratio_)
    else:
        # Should not happen with real PCA, but for safety
        return pca_full, max_possible
    ...
    pca_final: ReducerProtocol = PCA(n_components=n_components)
...
def apply_dimensionality_reduction(features: np.ndarray, labels: Optional[list] = None, 
    ...
    # ------------------------------------------------------------------
    # NEW: Pass-through branch (no dimensionality reduction)
    # ------------------------------------------------------------------
    if method == 'none':
        print("🔧 Dimensionality reduction DISABLED ('none'). Keeping original feature space.")
        class _PassThroughReducer:
            def fit(self, X): return self
            def transform(self, X): return X
        reducer = _PassThroughReducer()
...
def transform_new_features(features: np.ndarray, reducer: Union[Any, Tuple], method: str) -> np.ndarray:
    """
    ...
    """
    # Fast path when no dimensionality reduction is requested
    if method == 'none':
        return features

    if method == 'hybrid':
        # Type guard for hybrid method
        if not (isinstance(reducer, tuple) and len(reducer) == 2):
            raise TypeError("For hybrid method, reducer must be a tuple of (selector, pca_reducer).")
        selector, pca_reducer = reducer
        features_selected = selector.transform(features)
        transformed = pca_reducer.transform(features_selected)
    else:
        # Type guard for other methods
        if not hasattr(reducer, 'transform') or isinstance(reducer, tuple):
                raise TypeError(f"Reducer for method '{method}' must have a 'transform' method and not be a tuple.")
...
def extract_contiguous_context_blocks(df: pd.DataFrame, error_cols: list, 
    ...
    for i in range(1, len(df)):
        if df[context_col].iloc[i] != current_context:
            ...
            block_data = df.iloc[start_idx:i][error_cols].to_numpy()
    ...
    # Add the last block
    final_block_data = df.iloc[start_idx:][error_cols].to_numpy()
...
def create_fixed_windows_with_most_expensive_context(df: pd.DataFrame, error_cols: list, 
    ...
    for i in range(num_windows):
        ...
        window_data = window_df[error_cols].to_numpy()
        window_contexts = window_df[context_col].to_numpy()
...
def calculate_prices(
    ...
    # --- Feature Extraction (common for kmeans and xgboost) ---
    if any(m in METHODS_TO_RUN for m in ['kmeans', 'hybrid', 'xgboost']):
    ...
    if 'hybrid' in METHODS_TO_RUN:
        if kmeans_prices is not None and baseline_prices is not None:
            # Per user request: Use POST-PROCESSED kmeans and ORIGINAL baseline for hybrid calculation.
    ...
    # --- XGBoost prices (supervised) ---
    if 'xgboost' in METHODS_TO_RUN:
        if xgboost_model is not None and label_encoder is not None:
            # Predict context labels
            predicted_labels_encoded = xgboost_model.predict(reduced_features)
            predicted_contexts = label_encoder.inverse_transform(predicted_labels_encoded)
            
            # Map predicted contexts to prices
            xgboost_prices = np.array([get_price_for_context(ctx) for ctx in predicted_contexts])
            results['xgboost'] = xgboost_prices
        else:
            print("  Warning: XGBoost model not provided for pricing, skipping.")
...
def plot_pca_cluster_scatterplot(
    ...
    for i, centroid in pca_centroids.iterrows():
        ax.text(centroid['PC1'] + 0.05, centroid['PC2'] + 0.05, f'C{i}', fontsize=18, weight='bold', color='black')

    ax.set_title(f'PCA Scatter Plot of Clusters for {group_name}', fontsize=18)
    ax.set_xlabel('Principal Component 1', fontsize=18)
    ax.set_ylabel('Principal Component 2', fontsize=18)
    ax.legend(title='Context', bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=18)
...
def main():
    ...
    global RESULTS_DIR, FIXED_WINDOW_SIZE, TRAIN_VALIDATION_SPLIT, METHODS_TO_RUN
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Run the experiment for a single specified physics group (e.g., 'group_1'). Runs all if not specified."
    )
    # --- Device Selection ---
    parser.add_argument(
        "--device",
        ...
        help="Device for PyTorch operations ('auto' uses CUDA if available)."
    )
    # --- Path Configuration ---
    parser.add_argument(
        "--results-dir",
        type=str,
        default="results_experiment5",
        help="Directory to save all results, plots, and summaries."
    )

    # --- Experiment Parameters ---
    parser.add_argument("--fixed-window-size", type=int, default=FIXED_WINDOW_SIZE, help="Number of timesteps in each analysis window.")
    parser.add_argument("--train-val-split", type=float, default=TRAIN_VALIDATION_SPLIT, help="Fraction of files to use for training.")

    # --- Dimensionality Reduction ---
    parser.add_argument("--dim-redux-method", type=str, default=DIMENSIONALITY_REDUCTION['method'], choices=['none', 'intelligent_pca', 'umap', 'incremental_pca', 'feature_selection', 'hybrid'], help="Dimensionality reduction method (use 'none' to disable).")
    parser.add_argument("--dim-redux-target-variance", type=float, default=DIMENSIONALITY_REDUCTION['target_variance'], help="Target variance for intelligent PCA.")
    parser.add_argument("--dim-redux-max-components", type=int, default=DIMENSIONALITY_REDUCTION['max_components'], help="Max components for dimensionality reduction.")
    parser.add_argument("--dim-redux-min-components", type=int, default=DIMENSIONALITY_REDUCTION['min_components'], help="Min components for dimensionality reduction.")

    # --- Pricing Strategy ---
    parser.add_argument("--pricing-strategy-method", type=str, default=PRICING_STRATEGY['method'], choices=['weighted_average', 'hard_assignment', 'threshold_assignment', 'exponential_weighting'], help="Pricing strategy for KMeans.")
    parser.add_argument("--pricing-strategy-threshold-distance", type=float, default=PRICING_STRATEGY['threshold_distance'], help="Relative closeness threshold for threshold_assignment strategy.")

    # --- Hybrid Method ---
    parser.add_argument("--hybrid-alpha", type=float, default=HYBRID_PARAMS['alpha'], help="Incentive factor for hybrid method (alpha).")

    # --- Data Augmentation ---
    parser.add_argument("--aug-min-samples", type=int, default=AUGMENTATION_PARAMS['min_samples_per_class'], help="Minimum samples per class for data augmentation.")
    parser.add_argument("--aug-noise-level", type=float, default=AUGMENTATION_PARAMS['noise_level'], help="Noise level for data augmentation.")

    # --- Smart Post-Processing ---
    parser.add_argument("--post-process-floor-method", type=str, default="mode", choices=['mode', 'median', 'percentile'], help="Method to find baseline/floor in post-processing.")
    parser.add_argument("--post-process-gamma", type=float, default=0.6, help="Gamma value for power-law scaling in post-processing.")
    parser.add_argument("--post-process-dead-zone-k", type=float, default=3.0, help="Dead-zone multiplier (k) in post-processing.")
    
    # --- Quick Test ---
    parser.add_argument("--limit-files", type=int, default=None, help="Limit the number of train/validation files per group for a quick test run.")

    # --- Methods Selection ---
    parser.add_argument(
        "--methods-to-run",
        type=str,
        default=",".join(METHODS_TO_RUN),
        help="Comma-separated list of methods to run among kmeans, baseline, hybrid, xgboost (e.g., --methods-to-run xgboost)"
    )

    args = parser.parse_args()

    # --- Update METHODS_TO_RUN from CLI (must precede any use) ---
    #global METHODS_TO_RUN
    METHODS_TO_RUN = [m.strip() for m in args.methods_to_run.split(',') if m.strip()]
    valid_methods = {"kmeans", "baseline", "hybrid", "xgboost"}
    invalid_methods = [m for m in METHODS_TO_RUN if m not in valid_methods]
    if invalid_methods:
        raise ValueError(f"Invalid method(s) in --methods-to-run: {invalid_methods}. Valid options are: {sorted(valid_methods)}")

    # --- Update Global Configuration Dictionaries from Args ---
    ...
    # Update paths and simple parameters
    RESULTS_DIR = args.results_dir
    os.makedirs(RESULTS_DIR, exist_ok=True)
    FIXED_WINDOW_SIZE = args.fixed_window_size
    TRAIN_VALIDATION_SPLIT = args.train_val_split

    # Update dictionary configurations
    DIMENSIONALITY_REDUCTION.update({
        'method': args.dim_redux_method,
        'target_variance': args.dim_redux_target_variance,
        'max_components': args.dim_redux_max_components,
        'min_components': args.dim_redux_min_components,
    })

    PRICING_STRATEGY.update({
        'method': args.pricing_strategy_method,
        'threshold_distance': args.pricing_strategy_threshold_distance,
    })
    ...
    AUGMENTATION_PARAMS.update({
        'min_samples_per_class': args.aug_min_samples,
        'noise_level': args.aug_noise_level
    })

    # This one is not a global dict, so we create it locally for passing around
    post_process_config = {
        'floor_method': args.post_process_floor_method,
        'gamma': args.post_process_gamma,
        'dead_zone_k': args.post_process_dead_zone_k,
    }
    ...
    print("🚗 EXPERIMENT 5: Enhanced Billing Validation with Supervised Comparison")
    print("=" * 80)
    ...
    print(f"🎯 Dimensionality Reduction: {DIMENSIONALITY_REDUCTION['method']} (var={DIMENSIONALITY_REDUCTION['target_variance']})")
    print(f"💰 Pricing Strategy (KMeans): {PRICING_STRATEGY['method']} (thresh={PRICING_STRATEGY['threshold_distance']})")
    print(f"⚙️ Post-Processing: gamma={post_process_config['gamma']}, dead_zone_k={post_process_config['dead_zone_k']}")
    print(f"⚡ Device: {DEVICE}")
    print(f"⚡ Caching: Enabled for reconstruction error signals")
    print("=" * 80)
    ...
    for group_name, params in groups_to_process.items():
        ...
        group_csv_files = sorted(list(set(group_csv_files))) # Ensure uniqueness and order
        ...
        # --- Limit files for quick test run ---
        if args.limit_files is not None:
            print(f"--- QUICK TEST MODE: Limiting to {args.limit_files} train and {args.limit_files} validation files. ---")
            train_files = train_files[:args.limit_files]
            validation_files = validation_files[:args.limit_files]
        ...
        # 2. Generate/Load Reconstruction Error Signals for Training Data (with caching)
        print(f"  Generating/loading reconstruction error signals for training data...")
        train_dfs_with_errors = []
        
        for train_file in train_files:
            cache_path = os.path.join(RECONSTRUCTION_ERROR_CACHE_DIR, f"{os.path.basename(train_file)}.pkl")
            # --- Parallel-Safe Caching Logic ---
            if os.path.exists(cache_path):
                try:
                    df_with_errors = pd.read_pickle(cache_path)
                    print(f"    Loaded from cache: {os.path.basename(train_file)}")
                    train_dfs_with_errors.append(df_with_errors)
                    continue
                except Exception as e:
                    print(f"    Warning: Cache file {cache_path} seems corrupted, will regenerate. Error: {e}")

            # If cache doesn't exist or was corrupt, generate the data
            try:
                ...
                temp_cache_path = f"{cache_path}.{os.getpid()}.tmp"
                df_with_errors.to_pickle(temp_cache_path)
                
                try:
                    os.rename(temp_cache_path, cache_path)
                    print(f"    Processed and cached: {os.path.basename(train_file)}")
                except FileExistsError:
                    # Another process created it in the meantime. Clean up our temp file.
                    os.remove(temp_cache_path)
                    print(f"    Cache created by another process, using that one for: {os.path.basename(train_file)}")

                # Now that we're sure it exists, load it
                df_with_errors = pd.read_pickle(cache_path)
                train_dfs_with_errors.append(df_with_errors)
        ...
        assert isinstance(train_df_clean, pd.DataFrame)
        ...
        min_windows_per_context = min(len(w) for w in all_windows_grouped_by_context.values()) if all_windows_grouped_by_context else 0
        
        # --- Data Augmentation for Minority Classes ---
        print("  Checking for data augmentation needs...")
        
        augmented_windows = []
        augmented_contexts = []
        
        min_samples = AUGMENTATION_PARAMS['min_samples_per_class']
        
        for ctx, w_list in all_windows_grouped_by_context.items():
            augmented_windows.extend(w_list)
            augmented_contexts.extend([ctx] * len(w_list))
            
            if len(w_list) < min_samples:
                num_to_generate = min_samples - len(w_list)
                print(f"    Augmenting '{ctx}': {len(w_list)} -> {min_samples} (generating {num_to_generate})")
                
                for _ in range(num_to_generate):
                    # Pick a random sample from this class to augment
                    original_sample = random.choice(w_list)
                    
                    # Add Gaussian noise
                    noise = np.random.normal(
                        loc=0.0, 
                        scale=AUGMENTATION_PARAMS['noise_level'], 
                        size=original_sample.shape
                    )
                    
                    new_sample = original_sample + noise
                    augmented_windows.append(new_sample)
                    augmented_contexts.append(ctx)
        
        print(f"  Created a training set with {len(augmented_windows)} total windows after augmentation.")
        
        # Use the augmented data for training
        balanced_windows = augmented_windows
        balanced_contexts = augmented_contexts

        if not balanced_windows:
            print("  No training windows could be generated even after augmentation. Skipping group.")
            continue
        ...
        assert isinstance(all_windows_features, np.ndarray)
        ...
        assert isinstance(scaled_centroids, np.ndarray)
        ...
        # E. Fit KMeans model
        kmeans_model = KMeans(n_clusters=n_clusters, init=transformed_initial_centroids, n_init=1, random_state=42).fit(reduced_features) # type: ignore
        ...
        # === XGBoost Model Fitting (Supervised) ===
        xgboost_model, label_encoder = None, None
        if 'xgboost' in METHODS_TO_RUN and XGBOOST_AVAILABLE:
            print("  Fitting XGBoost classifier with Grid Search on the balanced dataset...")
            
            # A. Encode context labels
            label_encoder = LabelEncoder()
            balanced_labels_encoded = label_encoder.fit_transform(balanced_contexts)
            ...
            grid_search = GridSearchCV(
                estimator=xgb_clf,
                param_grid=XGBOOST_PARAMS['param_grid'],
                scoring=f1_scorer,
                cv=XGBOOST_PARAMS['grid_search_cv'],
                n_jobs=1, # CRITICAL FOR PARALLEL GRID SEARCH: Force serial execution
                verbose=1
            )
            
            # C. Run Grid Search
            grid_search.fit(reduced_features, balanced_labels_encoded)
            
            xgboost_model = grid_search.best_estimator_
            
            print(f"  XGBoost training complete. Best F1 (weighted) score: {grid_search.best_score_:.4f}")
            print(f"  Best parameters: {grid_search.best_params_}")
            
            # Save best params to file
            params_path = os.path.join(RESULTS_DIR, group_name, "xgboost_best_params.json")
            os.makedirs(os.path.dirname(params_path), exist_ok=True)
            with open(params_path, 'w') as f:
                import json
                json.dump(grid_search.best_params_, f, indent=4)
            print(f"  Best XGBoost parameters saved to {params_path}")
        ...
        for csv_file in train_files:
            ...
            try:
                ...
                cache_path = os.path.join(RECONSTRUCTION_ERROR_CACHE_DIR, f"{os.path.basename(csv_file)}.pkl")
                if os.path.exists(cache_path):
                    try:
                        run_df_with_errors = pd.read_pickle(cache_path)
                        print(f"      Loaded from cache: {os.path.basename(csv_file)}")
                    except Exception as e:
                        print(f"      Warning: Corrupted cache for {os.path.basename(csv_file)}, regenerating. Error: {e}")
                        os.remove(cache_path) # Remove corrupted file
                        run_df_with_errors = None
                else:
                    run_df_with_errors = None

                if run_df_with_errors is None:
                    df = pd.read_csv(csv_file)
                    df = process_data_for_autoencoder(df)
                    run_df_with_errors = generate_reconstruction_error_signals(df, autoencoder_model, autoencoder_scaler)
                    
                    # Use atomic write for caching
                    temp_cache_path = f"{cache_path}.{os.getpid()}.tmp"
                    run_df_with_errors.to_pickle(temp_cache_path)
                    try:
                        os.rename(temp_cache_path, cache_path)
                        print(f"      Processed and cached: {os.path.basename(csv_file)}")
                    except FileExistsError:
                        os.remove(temp_cache_path) # Another process won
                        print(f"      Cache for {os.path.basename(csv_file)} created by another process.")

                if run_df_with_errors.empty: continue

                # Create fixed-size windows from reconstruction error signals
                assert isinstance(run_df_with_errors, pd.DataFrame)
                error_data = run_df_with_errors[available_error_columns].to_numpy()
        ...
        for csv_file in validation_files:
            ...
            try:
                # Load data (with caching) - ensuring it's parallel-safe
                cache_path = os.path.join(RECONSTRUCTION_ERROR_CACHE_DIR, f"{os.path.basename(csv_file)}.pkl")
                if os.path.exists(cache_path):
                    try:
                        run_df_with_errors = pd.read_pickle(cache_path)
    ...
    summary_path = os.path.join(RESULTS_DIR, "full_evaluation_results_experiment5.csv")
    ...
    agg_path = os.path.join(RESULTS_DIR, "summary_evaluation_by_group_experiment5.csv")
    agg_metrics.to_csv(agg_path, index=False)
    print("\n" + "="*100)
    print("🎯 EXPERIMENT 5 RESULTS: Enhanced Features + Supervised XGBoost + Caching")
    print("="*100)
    print(f"📊 Methods: {METHODS_TO_RUN}")
    print("✍️ Analysis: Original cost for Baseline, Hybrid, XGBoost. Post-processed for KMeans.")
    ...
    print("    - Baseline/XGBoost/Hybrid: Original calculated costs")
    print("    - KMeans: Post-processed costs")
    print("  - XGBoost method: Fully supervised classification of contexts")
    print("="*100)
