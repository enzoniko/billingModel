    ...
    try:
        ...
        while process.is_running():
            ...
            try:
                ...
                logger.info(f"Training running... ({elapsed_time/60:.1f} min elapsed)")
        ...
        logger.info(f"SUCCESS: Training process completed! Total time: {total_time/3600:.1f} hours")
        return True
        
    except psutil.NoSuchProcess:
        logger.warning(f"ERROR: Process {pid} not found! It may have already completed.")
        return False
    except Exception as e:
        logger.error(f"ERROR: Error monitoring process: {e}")
        return False
...
def run_command(command: str, description: str, logger, timeout: int = 3600) -> Tuple[bool, str]:
    ...
    try:
        ...
        execution_time = time.time() - start_time
        logger.error(f"FAILED: {description} failed after {execution_time:.1f}s")
        ...
        execution_time = time.time() - start_time
        logger.error(f"ERROR: Unexpected error in {description} after {execution_time:.1f}s: {e}")
        return False, str(e)
...
def generate_enhanced_visualizations(logger) -> bool:
    ...
    for sim_id in all_sims_to_process:
        ...
        if success:
            ...
            logger.info(f"SUCCESS: Visualization {sim_id} completed ({successful_visualizations}/{len(all_sims_to_process)})")
        else:
            failed_visualizations.append(sim_id)
            logger.error(f"FAILED: Visualization {sim_id} failed")
    ...
    if failed_visualizations:
        logger.warning(f"Failed simulation IDs: {failed_visualizations}")
    
    # Return True if at least 80% succeeded
    success_rate = successful_visualizations / len(all_sims_to_process)
    if success_rate >= 0.8:
        logger.info(f"SUCCESS: Visualization phase completed! Success rate: {success_rate:.1%}")
        return True
    else:
        logger.error(f"FAILED: Visualization phase had too many failures. Success rate: {success_rate:.1%}")
        return False
...
def run_experiment2(logger) -> bool:
    ...
    if success:
        logger.info("SUCCESS: Experiment 2 completed successfully!")
        return True
    else:
        logger.error("FAILED: Experiment 2 failed!")
        return False
...
def generate_final_summary(logger, start_time: float, results: Dict[str, bool]):
    ...
    logger.info(f"Total execution time: {total_time/3600:.1f} hours")
    ...
    if all_success:
        ...
        logger.warning(f"\nSOME TASKS FAILED: {failed_tasks}")
...
def main():
    ...
    try:
        ...
        if not training_success:
            logger.warning("Training monitoring failed, but continuing with pipeline...")
